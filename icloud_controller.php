...
class Icloud_controller extends Module_controller
{
    ...
    **/
    ...
    /**
    * Retrieve back_to_my_mac_enabled in json format
    *
    * @return void
    * @author tuxudo
    **/
    public function get_back_to_my_mac()
    {
        $obj = new View();
        if (! $this->authorized()) {
            $obj->view('json', array('msg' => 'Not authorized'));
        ...
        $queryobj = new Icloud_model();
        $sql = "SELECT COUNT(1) as total,
                        COUNT(CASE WHEN `back_to_my_mac_enabled` = 1 THEN 1 END) AS 'yes',
                        COUNT(CASE WHEN `back_to_my_mac_enabled` = 0 THEN 1 END) AS 'no'
                        from icloud
                        LEFT JOIN reportdata USING (serial_number)
                        WHERE
                            ".get_machine_group_filter('');
